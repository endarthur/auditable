#!/usr/bin/env node
// Bundles ext/vfs/src/ ES modules into a single index.js
...
const output = header + '\n' + chunks.join('\n\n') + '\n\nexport { VFS, VFSError, CommentBackend, MemoryBackend, path };\n';

const outPath = path.join(__dirname, 'index.js');
fs.writeFileSync(outPath, output);
const size = fs.statSync(outPath).size;
console.log(`Built ext/vfs/index.js (${(size / 1024).toFixed(1)} KB)`);
