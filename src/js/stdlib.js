...
// RGB ↔ linear RGB
function _rgbToLinear(r, g, b) {
  return [_srgbToLinear(r), _srgbToLinear(g), _srgbToLinear(b)];
}
function _linearToRgb(lr, lg, lb) {
  return [_linearToSrgb(lr), _linearToSrgb(lg), _linearToSrgb(lb)];
}

// linear RGB ↔ OKLAB (via LMS)
function _linearToOklab(lr, lg, lb) {
...
function _rgbToOklab(r, g, b) {
  const [lr, lg, lb] = _rgbToLinear(r, g, b);
  return _linearToOklab(lr, lg, lb);
}
function _rgbToOklch(r, g, b) {
  const [L, a, bb] = _rgbToOklab(r, g, b);
  return _oklabToOklch(L, a, bb);
}
function _oklabToRgb(L, a, b) {
  const [lr, lg, lb] = _oklabToLinear(L, a, b);
  return _linearToRgb(lr, lg, lb);
}
...
function _makeColor(r, g, b, a) {
  ...
  return Object.freeze({
    r, g, b, a,
    hsl() { const [h, s, l] = _rgbToHsl(r, g, b); return { h, s, l }; },
    oklab() { const [L, oa, ob] = _rgbToOklab(r, g, b); return { L, a: oa, b: ob }; },
    oklch() { const [L, C, oh] = _rgbToOklch(r, g, b); return { L, C, h: oh }; },
    linear() { const [lr, lg, lb] = _rgbToLinear(r, g, b); return { r: lr, g: lg, b: lb }; },
    lighten(amt) {
      const [L, ca, cb] = _rgbToOklab(r, g, b);
      const [nr, ng, nb] = _oklabToRgb(Math.min(1, L + amt), ca, cb);
      return _makeColor(nr, ng, nb, a);
    },
    darken(amt) {
      const [L, ca, cb] = _rgbToOklab(r, g, b);
      const [nr, ng, nb] = _oklabToRgb(Math.max(0, L - amt), ca, cb);
      return _makeColor(nr, ng, nb, a);
    },
    saturate(amt) {
      const [L, C, h] = _rgbToOklch(r, g, b);
      const [ca, cb] = _oklchToOklab(L, Math.max(0, C + amt), h).slice(1);
      const [nr, ng, nb] = _oklabToRgb(L, ca, cb);
      return _makeColor(nr, ng, nb, a);
    },
    desaturate(amt) {
      const [L, C, h] = _rgbToOklch(r, g, b);
    ...
    rotate(deg) {
      const [L, C, h] = _rgbToOklch(r, g, b);
      const [_, ca, cb] = _oklchToOklab(L, C, h + deg);
      const [nr, ng, nb] = _oklabToRgb(L, ca, cb);
      return _makeColor(nr, ng, nb, a);
    },
    mix(other, t = 0.5) {
      const c2 = other.r !== undefined ? other : color(other);
      const [L1, a1, b1] = _rgbToOklab(r, g, b);
      const [L2, a2, b2] = _rgbToOklab(c2.r, c2.g, c2.b);
      const [nr, ng, nb] = _oklabToRgb(
...
function color(input) {
  ...
  if (m) {
    const [r, g, b] = _hslToRgb(+m[1], +m[2], +m[3]);
