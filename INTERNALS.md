...
- **`@gcu/vec` v0.1.1 shipped.** Pure-JS TypedArray-based numerical library; lightweight NumPy alternative. NdArray + NumPy-style broadcasting, ~50 element-wise ops, reductions with axis, small dense linear algebra (LU solve, Cholesky, lstsq, eigSym3 via Cardano closed-form, eigSym via Jacobi), shape ops (concat/stack/diag/outer/tril/triu/slice). Adder bridge wraps NdArray as VecArray with Python dunder methods (registers as `vec`). 8.4 KB minified+gzipped, zero runtime deps. 143 vec-specific tests + 38 v0.1.1 additions = 181 vec tests. Ships at `ext/vec/` with own package.json, ready to `npm publish`.
