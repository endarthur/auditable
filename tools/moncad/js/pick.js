...
// Distance from world point p to a feature's geometry (Infinity if not hittable).
export function featureDist(f, p) {
...
export function pickWindow(features, box) {
  const out = [];
  for (let i = 0; i < features.length; i++) {
    const bb = featureBounds(features[i]); if (!bb) continue;
