...
const HELP = {
  start: ['Getting started',
    ...
    + `<b>Understand & check the data</b> — <b>Σ stats</b> (toolbar) summarizes every column at once; <b>Data → Group by…</b> aggregates by domain (with weighting); <b>Data → Data quality…</b> flags silent problems (leading-zero codes, missing-value sentinels, junk in numeric columns). See <i>Analysis &amp; quality</i>.<br><br>`
  ...
  filter: ['Filter syntax',
    ...
    + `<b>Functions</b> <code>round int abs sqrt log exp pow min max clamp bin if(c, a, b)</code> · casts <code>coalesce ifnum</code> · tests <code>isnum isnan</code> — e.g. <code>sqrt(au) > 0.5</code>. As a calculated column, <code>bin(grade, 0.5)</code> buckets a number to its bin's lower edge — group by it for grade-tonnage by cutoff.<br><br>`
